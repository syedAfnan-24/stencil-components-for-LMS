...
export class ClientBorrow {
  ...
  renderBorrow() {
    ...
    const filteredBorrows: Borrow[] = this.borrows.filter(item => item.username === this.name);
          ...
          <tbody>
            {filteredBorrows.map(item => (
              <tr key={item.id}>
                ...
                <td><button id="borrow-btn" onClick={this.returnBook.bind(this, item.id, item.bookname, item.borrowdate)}>Return</button></td>
              </tr>
            ))}
          </tbody>
        </table>
